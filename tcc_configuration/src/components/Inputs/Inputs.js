...
const Inputs = props => {
    ...
    const initPointsList = () => {
        ...
        for (var input in configuration["inputs"]) {
            if (configuration["inputs"][input]["topic"] !== "") {
                let devIndex = findDeviceIndex(configuration["inputs"][input]["topic"].split("/")[3], devices)
                if (devIndex !== -1) {
    ...
    const handleTopicChange = (e, index) => {
        if (configuration["campus"] === "" || configuration["building"] === "") {
            ...
            let config = configuration;
            inputArray[index]["topic"] = `devices/${config["campus"]}/${config["building"]}/${e.target.value}/all`
